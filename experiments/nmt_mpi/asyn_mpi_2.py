...
def master():
    ...
    train_context = train_mpi.before_train_master()
    synchronize_tag(tags.DONE)
    ...
    while True:
        ...
        if interrupt:
            train_mpi.after_train_master(train_context)
            return
...
def worker():
    ...
    while True:        
        ...
        if tag == tags.BEFORE_TRAIN:
            train_context = train_mpi.before_train_worker()
            comm.send(None, dest=0, tag=tags.DONE)

        if tag == tags.TRAIN_BATCHES:
            vals = data
            train_mpi.set_parameter(context,vals)
            success = train_mpi.train_batches_worker(train_context)
            vals = get_parameter(context)
            if not success:
                vals = None
